    ...
    public class SendServer
    {
        ...
        public Result SwitchPageAll(string storeCode, int page)
        {
            ...
            catch (Exception ex)
            {
                ...
                public Result DisplayBarcodeByTag(string storeCode, string tagID)
        {
            if (!RegStoreCode.IsMatch(storeCode))
                return Result.InvalidStoreCode;

            if (!RegTagID.IsMatch(tagID))
                return Result.InvalidTagID;

            try
            {
                var aps = GetAPs(storeCode);
                var result = Server.Instance.DisplayBarcode(storeCode, aps.FirstOrDefault(), 0xFFFF); // Default token is 0.
                if (result == SdkResult.OK)
                {
                    CoqAPEvents.Enqueue(new APStatusEventArgs(storeCode, aps.FirstOrDefault(), APStatus.Working));
                }
                _logger.LogInformation($"[Cronus]Display barcode, {storeCode}-{aps.FirstOrDefault()}:{result}.");

                return Result.OK;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[Cronus]DisplayBarcodeByTag_Error");
                return Result.Error;
            }
        }
        /// <summary>
